...
<div id="adminTree">
        ...
        <div class="secondChildRow">
          <div class="secondChildDeta" v-for="secondDeta in fistData.child" :key="secondDeta.key">
            <div class="headRow" style="margin-top:0;margin-bottom:0.1rem;">
              <p class="secondChildDetaP">{{secondDeta.username}}</p>
              <p class="begatsPRow begatsPRowfistC global_txtOver">{{secondDeta.name}}</p>
              <p class="begatsPRow begatsPRowfistC global_txtOver">{{secondDeta.mobile}}</p>
              <div class="begatsChangeBtn" @click="ajaxBegatsDeta(secondDeta.username, false)" style="height:0.2rem;line-height:0.2rem;padding-right:0.05rem">+</div>
            </div>
            <div class="branchImgBox">
            ...
            <div class="lastChildRow">
              <div class="headRow" v-for="lastDeta in secondDeta.child" :key="lastDeta.key" style="margin-top:0;margin-bottom:0.1rem;height: 0.5rem;
                        width: 0.38rem;">
                <p style="padding-top:0.09rem;font-size:0.12rem" class="global_txtOver">{{lastDeta.username}}</p>
                <div class="begatsChangeBtn" @click="ajaxBegatsDeta(lastDeta.username, false)" style="height:0.1rem;line-height:0.1rem;padding-right:0;font-size:0.16rem;">+</div>
              </div>
            </div>
          </div>
        </div>
  ...
  <div class="searchIdBox">
    <el-input placeholder="请输入您要搜索的用户名" icon="search" v-model="searchId" :on-icon-click="handleIconClick">
    </el-input>
...
#adminTree .headRow .begatsChangeBtn {
  position: absolute;
  height: 0.36rem;
  bottom: 0;
  left: 0;
...
#adminTree .headRow .secondChildDetaP {
